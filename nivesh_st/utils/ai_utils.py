...
def get_working_model(api_key: str) -> str:
    """Find the best available model for this API key."""
    try:
        ...
        for model_id in MODEL_CASCADE:
            try:
                ...
                err = str(e).lower()
                if "not found" in err or "deprecated" in err or "no longer available" in err or "404" in err:
                    continue  # try next model
                elif "quota" in err or "rate" in err or "429" in err:
                    return model_id  # quota error means model exists, just rate-limited
                elif "permission" in err or "403" in err:
                    return model_id  # model exists, API key issue
                break
    except Exception:
        pass
...
def get_model_name() -> str:
    """Returns current model. If user set one, use it. Otherwise find best available."""
