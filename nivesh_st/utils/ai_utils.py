...
def get_model_name() -> str:
    """Returns current model. Clears deprecated models from session state."""
    user_model = st.session_state.get("gemini_model")
    # Clear deprecated model from session state automatically
    if user_model and user_model in DEPRECATED_MODELS:
        del st.session_state["gemini_model"]
        user_model = None
    if user_model:
        return user_model
    # Auto-discover best working model
    try:
        return get_working_model(get_key())
    except Exception:
        return DEFAULT_MODEL
...
def generate(prompt: str, temperature: float = 0.5, max_tokens: int = 16000,
...
Format as a structured research note with specific numbers. Be concise but data-rich."""
    ...
    return generate(prompt, temperature=0.2, max_tokens=16000, use_search=True)
...
3 specific observations about current price action and what to watch."""
    ...
    return generate(prompt, temperature=0.3, max_tokens=16000)
...
### 🧮 HOW TO INVEST (SIP/lump sum, levels, profit booking)"""
    ...
    return generate(prompt, temperature=0.4, max_tokens=16000)
  ...
  "time_horizon": "12 months"
    ...
    try:
        ...
        raw_search = generate(prompt + "\n\nIMPORTANT: Return ONLY valid JSON.",
                              temperature=0.1, max_tokens=16000, use_search=True)
