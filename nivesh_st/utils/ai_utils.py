...
Default: gemini-3.1-pro-preview (2026 — most intelligent)
...
DEFAULT_MODEL = "gemini-3.1-pro-preview"  # Gemini 3.1 Pro — most intelligent (2026)
...
# Gemini 2026 models — newest first
MODEL_CASCADE = [
    "gemini-3.1-pro-preview",        # Most intelligent — complex reasoning (2026)
    "gemini-3.1-flash-lite-preview",  # Fastest, lowest cost (2026)
    "gemini-3.0-flash",               # High-speed general purpose
    "gemini-2.5-pro",            # 2025 stable fallback
    "gemini-2.5-flash",          # 2025 stable fallback
    "gemini-1.5-pro",            # Legacy stable fallback
]

def get_working_model(api_key: str) -> str:
    """
    Find the best available Gemini model for this API key.
    Tries models in CASCADE order. Returns first one that works or exists.
    A model 'exists' if error is quota/rate/permission — not 404/deprecated.
    """
    try:
        ...
        for model_id in MODEL_CASCADE:
            try:
                ...
                return model_id  # successfully responded
            except Exception as e:
                err = str(e).lower()
                # Model doesn't exist or deprecated — skip to next
                if any(x in err for x in ["not found","deprecated","no longer available",
                                           "404","invalid model","not supported"]):
                    continue
                # Model EXISTS but has quota/permission issue — still use it
                # (quota resets, permission is key-level not model-level)
                if any(x in err for x in ["quota","rate","429","403","permission",
                                           "resource_exhausted","resource exhausted"]):
                    return model_id
                # Unknown error — still try next model instead of breaking
                continue
    except Exception:
        pass
    return DEFAULT_MODEL

# Models known to be deprecated — auto-clear if stored in session state
DEPRECATED_MODELS = {
    "gemini-2.0-flash", "gemini-2.0-flash-exp", "gemini-2.0-pro",
    "gemini-1.0-pro", "gemini-1.0-pro-latest", "gemini-1.0-ultra",
}

def get_model_name() -> str:
    """
    Returns the best available Gemini model.
    Always returns DEFAULT_MODEL (first in cascade) — let generate() handle fallback.
    Clears deprecated models from session state.
    """
    user_model = st.session_state.get("gemini_model")
    if user_model and user_model in DEPRECATED_MODELS:
        del st.session_state["gemini_model"]
        user_model = None
    if user_model and user_model not in DEPRECATED_MODELS:
        return user_model
    return DEFAULT_MODEL
...
def generate(prompt: str, temperature: float = 0.5, max_tokens: int = 65536,
             use_search: bool = False) -> str:
    """
    Generate a response from Gemini.
    Tries EVERY model in MODEL_CASCADE until one works — never fails silently.
    Self-healing: if a model is deprecated/quota/closed, auto-tries next.
    """
    last_err = ""
    for model_id in MODEL_CASCADE:
        for with_search in ([use_search, False] if use_search else [False]):
            try:
                config_args = dict(
                ...
                # Success — update session state to remember this working model
                try:
                    import streamlit as _st
                    _st.session_state["gemini_model"] = model_id
                except Exception:
                    pass
                return resp.text
            except Exception as e:
                err = str(e).lower()
                last_err = str(e)
                # Hard skip — model doesn't exist
                if any(x in err for x in ["404","not found","no longer available",
                                           "deprecated","invalid model"]):
                    break  # skip remaining search variants for this model, try next
                # Soft skip — client closed, clear cache and retry once
                if "closed" in err:
                    try: _get_cached_client.clear()
                    except Exception: pass
                    try:
                        client2 = genai.Client(api_key=get_key())
                        ...
                        break
                # Quota/rate limit — model works but quota hit, still try next model
                if any(x in err for x in ["429","quota","resource_exhausted","rate"]):
                    break  # try next model
                # Unknown — try next search variant, then next model
                continue
    return f"⚠️ All Gemini models unavailable. Last error: {last_err[:200]}"
...
def list_models() -> list:
    try:
        ...
        return [
            {"id":"gemini-3.1-pro-preview",        "name":"Gemini 3.1 Pro ⭐ (2026 — Most intelligent)"},
            {"id":"gemini-3.1-flash-lite-preview","name":"Gemini 3.1 Flash Lite Preview (2026 — Fastest)"},
            {"id":"gemini-3.0-flash",              "name":"Gemini 3 Flash (2026 — Fast & cost-effective)"},
            {"id":"gemini-2.5-pro",       "name":"Gemini 2.5 Pro (2025 stable fallback)"},
            {"id":"gemini-2.5-flash",     "name":"Gemini 2.5 Flash (2025 stable fallback)"},
            {"id":"gemini-1.5-pro",       "name":"Gemini 1.5 Pro (Legacy fallback)"},
        ]

def chat_response(history: list, question: str) -> str:
    """Multi-turn chat — tries all models in MODEL_CASCADE."""
    contents = [
    ...
    for model_id in MODEL_CASCADE:
        try:
            ...
            err = str(e).lower()
            if any(x in err for x in ["404","not found","no longer available","deprecated",
                                       "closed","429","quota","resource_exhausted"]):
                continue
            break
    return generate(question, max_tokens=65536, use_search=True)
...
3 specific observations about current price action and what to watch."""
    ...
    return generate(prompt, temperature=0.3, max_tokens=65536)
...
### 🧮 HOW TO INVEST (SIP/lump sum, levels, profit booking)"""
    ...
    return generate(prompt, temperature=0.4, max_tokens=65536)
  ...
  "time_horizon": "12 months"
    ...
    try:
        ...
        raw_search = generate(prompt + "\n\nIMPORTANT: Return ONLY valid JSON.",
                              temperature=0.1, max_tokens=65536, use_search=True)
