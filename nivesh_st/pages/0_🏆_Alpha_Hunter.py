...
from utils.ai_utils import get_model_name, generate, get_auto_macro_context, get_investment_levels
...
if deep_btn and selected:
    ...
    with tab_invest:
        ...
        with st.spinner("Gemini researching macro + company context..."):
            macro_ctx = get_auto_macro_context(selected)
    ...
    with tab_research:
        with st.spinner("Fetching latest macro + company news via Gemini web search..."):
            ctx = get_auto_macro_context(selected)
        st.markdown(ctx)
